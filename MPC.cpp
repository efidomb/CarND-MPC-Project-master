...
double ref_cte = 0;
double rev_epsi = 0;
double ref_v = 100; // I don't know what value should I peak. hope that's OK.
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
	...
	// fg supposed to be bigger than the fg as he coming by 1: the cost in fg[0]
	  ...
	  for (int t = 0; t < N; t++) {
		  fg[0] += CppAD::pow(vars[cte_start + t], 2); // it could be that the CppAD is not needed because of the using statment at the top on the page. same for the next few lines.
		  fg[0] += CppAD::pow(vars[epsi_start + t], 2); // also, tune the coeffs for the costs.
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // but what about the other vars_lowerbound and vars_upperbound? what should I initialize them to? to 0?
