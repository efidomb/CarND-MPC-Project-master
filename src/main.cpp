			...
			if (s != "") {
				...
				if (event == "telemetry") {
					...
					*/
					...
					Eigen::VectorXd coeffs = polyfit(xvals, yvals, 3); // should it be an auto type?
																	   // Calculate errors
					double cte = polyeval(coeffs, 0);
					double epsi = -atan(coeffs[1]);

					// Use the values from the simulator
					...
					steer_value = vars[0] * - 1; // this line probably needs some modification like: vars[0] / deg2rad(25) * Lf, as shown in the video. but it made more sence to do it in the MPC.cpp file.
